...
\hrule 
\begin{abstract}
  Making a payment in a privacy aware payment channel network is currently achieved by trying several payment paths until one succeeds. Subject to the network size, it can take several minutes for a payment to complete.
  We introduce a network imbalance measure and provide empirical evidence that a better balance of the network leads to higher a success rate when making random payments, which in turn reduce the payment time.
  ...
  Channel partners can share information with each other and they can engage in fee-free rebalancing operations if that operation is beneficial for the balance of every participant along the rebalancing circle.
  Thus, participants will only be able to improve their balance if this operation does not worsen the balance of other nodes. This ensures that the heuristic improves the balance of the network.
  We demonstrate empirically on synthetic random networks that the balance measure is well defined and that the proposed algorithm quickly converges to a balanced enough network that supports random payments without failures.
\end{abstract}
