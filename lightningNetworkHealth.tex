...
Both queries will allow the nodes to signal that they will be willing to participate in a rebalancing operation for up to $x$ tokens.
...
Nodes will drop the routing fees for this circular path as it will improve the balance of each node and provide a direct benefit instead of a routing service.
...
Also nodes don't have to answer to the queries truthfully and could always introduce some fuzziness for privacy reasons. 
...
First of all nodes can probe for rebalancing circles that are beneficial for itself.
Second s nodes on the way would only fulfill the fee free rebalancing request if it also improves their balance this algorithm could work without actively sharing information in the friend of a friend network.
...
\end{enumerate}
After each step we track all channel balance coefficients, local helth of nodes and the balance of the network.
