...
\graphicspath{{img/}}
\title{Improving path finding in the Lightning Network by proactive rebalancing of payment channels with an agent based heuristic}
...
As the funds and balances of channels are not globally known we introduce a greedy agent based heuristic with which every node despite the uncertainty can improve its own local balance.
In an empirical simulation on a recent snapshot of the Lightning Network we show that the imbalance distribution of the network has a Kolmogorov Smirnoff distance of $0.74$ in comparison to the imbalance distribution after the heuristic is applied.
